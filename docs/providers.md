...
Pi applies exact MCP preapproval through the adapter's tool approval broker. Mark every server named by a tool policy as approval-gated, then let the generated Paseo extension claim only the exact granted server/tool pairs. This keeps ungranted tools on the same server behind the adapter's normal approval path. Reject agent creation when a tool policy is present and the adapter is unavailable.
